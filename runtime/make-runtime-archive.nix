...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p archive/bin archive/config archive/installer archive/runtime archive/tools archive/docs archive/bootstrap archive/channel archive/tests/termux
    ...
    install -Dm755 runtime/activate-config.sh archive/runtime/activate-config.sh
    install -Dm644 config/flake.nix archive/config/flake.nix
    install -Dm644 config/flake.lock archive/config/flake.lock
